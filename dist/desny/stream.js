...
async function getStream({ link, providerContext, }) {
    ...
    try {
        ...
        // STEP 2: Build headers specific to Disney (Ott: 'hs')
        const headers = {
            ...newTvBaseHeaders,
            "Ott": "hs", // 'hs' is passed in Kotlin for DisneyStudioProvider
            "Usertoken": ""
        };
        // STEP 3: Request the player endpoint
        const res = await axios.get(`${apiBase}/newtv/player.php?id=${id}`, { headers });
        const data = res.data;
        // STEP 4: Parse response and provide stream
        if ((data === null || data === void 0 ? void 0 : data.status) === "ok" && (data === null || data === void 0 ? void 0 : data.video_link)) {
            streams.push({
                server: `Disney Auto`,
                link: data.video_link, // Usually an M3U8 link
                type: "m3u8",
                headers: {
                    "User-Agent": "Mozilla/5.0 (Android) ExoPlayer",
                    "Accept": "*/*",
                    // Interceptor in Kotlin adds this cookie specifically for .m3u8
                    "Cookie": "hd=on",
        ...
        console.error("Disney stream extraction failed:", e.message);
    }
    return streams;
}
