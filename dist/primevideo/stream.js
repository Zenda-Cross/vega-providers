...
async function getStream({ link, providerContext, }) {
    ...
    // link format: "ID|Title" - we just need the ID to fetch streams.
    const [id] = link.split("|");
    const streams = [];
    try {
        // STEP 1: Get dynamic base API URL
        const apiBase = await resolveApiUrl(axios);
        // STEP 2: Build headers specific to Prime Video (Ott: 'pv')
        const headers = {
            ...newTvBaseHeaders,
            "Ott": "pv", // 'pv' is what Prime Video expects according to Kotlin
            "Usertoken": ""
        };
        ...
        console.error("PrimeVideo stream extraction failed:", e.message);
    }
    return streams;
}
